...
function updateImg() {
	...
	if(valideImg!=0) {
		...
		if(displayImg==1) {
			...
			urlImg = '<?=$_SESSION['home']?>images/boitiers/' + boitier.options[boitier.selectedIndex].text + '.jpg';
			alert(urlImg);
		} else {
			displayImg=1;
			urlImg = '<?=$_SESSION['home']?>images/boitiers/' + boitier.options[boitier.selectedIndex].text + '.gif';
			alert(urlImg);
		}
		//alert(urlImg);
...
function resetImg() {
	...
	if(displayImg==2) {
		valideImg = 0;
		setImgBoitier("<?=$_SESSION['home']?>images/spacer.gif", false);
	} else {
		valideImg = 1;
		//displayImg = 1;
		updateImg();
...
if(isset($_POST['actionDataImg'])) {
	...
	if($ext!="png") {
		...
		if($imgGrp==1) {
			...
			addMEssage("Image ajout�e pour cet article uniquement");
		} else if($imgGrp==2) {
			// image pour genre/type
			$target = "../images/articles/".$IDGenreImg."_".$IDTypeImg."_0.".$ext;
			if(!is_file($target)) {
				// pas de fichier 0
				// v�rifier si fichiers 1 ou suivants existent
				$cntImg = 1;
				...
				if($cntImg==1) {
					...
					move_uploaded_file( $_FILES['imgDownload']['tmp_name'], $target);
					addMEssage("Premi�re image ajout�e pour cette cat�gorie");
				} else {
					// au moins un fichier d�j� existant -> on ajoute avec un nouveau num�ro
					move_uploaded_file( $_FILES['imgDownload']['tmp_name'], $target1);
					addMEssage("Image no ".$cntImg." ajout�e pour cette cat�gorie");
				}
			} else {
				// fichier 0 existe d�j� -> renommer en fichier 1
				$target1 = "../images/articles/".$IDGenreImg."_".$IDTypeImg."_1.".$ext;
...
REQ;
    $resultat =  mysqli_query($connexionDB,$requete);
	if($resultat) {
		addMEssage("Nouveau sch�ma ajout�");
	} else {
...
REQ;
  $resultat =  mysqli_query($connexionDB,$requete);
    if($resultat) {
	addMessage("Composant mis � jour");
	mysqli_query($connexionDB,'COMMIT');
    } else {
	addMEssage("<font color=red>Impossible de mettre � jour le composant </font>");
    }
  // test pour action suppl�mentaires
  if(!empty($IDReferenceNew)) {
	// nouveau footprint sur les r�f�rence existantes
	$actionFoot="Ajouter";
  }
    if(!empty($ReferenceNew)) {
	// nouveau footprint avec nouvelle r�f�rence
...
//echo "$requete";
  ...
  if($actionFourn=="Supprimer") {
	...
	$resultat =  mysqli_query($connexionDB,$requete);
	if($resultat) {
		addMEssage("Fournisseur supprim�");
		mysqli_query($connexionDB,'COMMIT');
	} else {
		addMEssage("<font color=red>Impossible de supprimer le fournisseur</font>");
	}
...
//echo "$requete";
    $resultat =  mysqli_query($connexionDB,$requete);
	if($resultat) {
		addMEssage("Nouveau Footprint ajout�");
		$actionFoot="Ajouter";
...
REQ;
  ...
  if($actionStock=="Supprimer") {
	...
	$resultat =  mysqli_query($connexionDB,$requete);
	if($resultat) {
		addMEssage("Emplacement supprim�");
		mysqli_query($connexionDB,'COMMIT');
	} else {
	...
	switch ($value) {
		...
		case 1:
			if($pos!=0) {
				...
				// si une seule position utilis�e, on coupe � 45
...
REQ;
  ...
  $ligne = mysqli_fetch_assoc($resultat);
	...
	/* selection des lignes d'impression pour �tiquettes */
	$selected11 = "";
	$selected21 = "";
	$selected31 = "";
	$selected41 = "";
...
<!--br><h2>D�tail de l'article</h2-->
<br><div id='corners'>
<div id='legend'>D�tail de l'article</div>
...
<?php if($action!="Nouveau") echo "<td align='right' width='40'><input type='radio' name='printC1' value='1' $selectedC11></td><td align='left' width='40'><input type='radio' name='printC2' value='1' $selectedC12></td><td align='right' width='40'><input type='radio' name='print1' value='1' $selected11></td><td align='left' width='40'><input type='radio' name='print2' value='1' $selected12></td>"; ?>
</tr>
...
<tr><td>Caract�ristiques:</td><td><input type="texte" name="Caracteristiques" value="<?= htmlspecialchars($ligne['Caracteristiques']) ?>" size="60"></td>
...
if($action!="Nouveau") {
	if($app_section=='ELT') {
	  /* Boitier */
	  echo "<tr><td>Bo�tier:</td><td ><select id='IDBoitier' name='IDBoitier' onChange='updateImg()'><option></option>";
	  ...
	  echo "</select></td>";
	  //echo "<td colspan='2' rowspan='3'><a id='refBoitier' href='#'><img id='imgBoitier' src='".$_SESSION['home']."images/spacer.gif' onerror='resetImg();' width='100' height='80' ></a><script>updateImg();</script></td>";
...
if($action!="Nouveau") {
  ...
  echo "<tr><th>R�f�rence fournisseur</th><th>Fournisseur</th><th>Fabriquant</th><th>Prix/pce</th><th align='center'>Derni�re commande</th><th align='center'>Quantit�</th><th></th></tr>";
  $requete = <<<FOURN
...
//echo $requete;
  $resultat =  mysqli_query($connexionDB,$requete);
    while ($fournLigne = mysqli_fetch_assoc($resultat)) {
		if($fournLigne['IDCommande']!=$lastIDCom) {
			...
			if(isset($fournLigne['DateReception'])) {
			...
			if(!empty($dataSheet)) {
				echo "<a href='$dataSheet' target='_datasheet'><img src='".$_SESSION['home']."images/pdf.jpg' width='16' height='16' align='absmiddle' onmouseover=\"Tip('Lien sur datasheet du fournisseur')\" onmouseout='UnTip()'></a>&nbsp";
			}
