...
//if(!isset($login)) {
//	// utilisateur par d�faut -> invit�
//	$login = DBUser;
//	$mdp = DBPwd;
//	$_SESSION['user_nom'] = "Invit�";
	   ...
	   while ($row = mysqli_fetch_array($result) ) {
		   ...
		   if($cat==CatElectro || $cat==CatAtelier) {
			...
			while ($soft = mysqli_fetch_array($resultSoft)) {
				...
				if($soft['StatusLienDirect']) {
					echo "<a href='$soft[LienDirect]'>T�l�charger</a>";
				} else {
					echo "T�l�chargement indisponible <img src='/iconsFam/link_break.png'>";
				}
		...
		//echo "</p>";
		   ...
		   if(!$existDoc && !$child && !hasAdminRigth()) {
				// aucuns documents, on cache le bloc concern�
...
<div class="post">
	<br>Cat�gorie: <select name="niveau1" onChange='submit();'><option value="">Tous</option><?=getOptionsCat($cat,$niveau1)?></select>
	<?php
	$root = $cat;
	$filtre = '';
	if(!empty($niveau1)) {
	...
	if(hasAdminRigth()) {
       	echo "<img src='/iconsFam/folder_add.png' align='absmiddle' onmouseover=\"Tip('Ajouter cat�gorie')\" onmouseout='UnTip()' onClick='toggle(\"newDossier$cat\")'>";
       	echo "<div style='background-color: #EEEEEE; padding:10px 5px 10px 10px;' id='newDossier$cat'><input type='texte' name='newDossier' id='name$cat' value=''><input type='button' name='AjouterDossier' value='Ajouter cat�gorie' onclick='submitNewDossier(\"$cat\")'></div><script>document.getElementById(\"newDossier$cat\").style.display = \"none\"</script>";
	}
	 ?>
	<br><br>
	<div id='block<?=$root?>'>
