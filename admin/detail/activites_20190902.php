...
if(isset($_POST['AjoutActivite'])) {
	...
	//echo $heures."-".$heureDebut."-".$heureFin;
...
if(isset($_POST['activite'])&&!empty($_POST['activite'])) {
	...
	//echo $heuresMaj."-".$heureDebutMaj."-".$heureFinMaj;
	if($dateCheck<$lundi || $dateCheck>$vendredi) {
...
if(empty($IDQuery)) {
	...
	$requete = "SELECT * FROM journal jou join theme th on jou.IDTheme=th.IDTheme where IDEleve = $IDEleve and (DateJournal between '".date('Y-m-d', $lundi)."' and '".date('Y-m-d', $vendredi)."') order by jou.IDTheme, DateJournal, HeureDebut";
} else {
	if($IDQuery==2) {
		// tout 
...
echo "<tr><th width='175'>Thème</th><th width='90'>Date</th><th width='85'>Heures</th><th>Activités</th><th width='25' align='right'>";
if(empty($IDQuery)) {
...
echo "<tr newActivite='1' style='display:none'><td valign='top' colspan='5' height='30'><select name='IDTheme'>".$optionAPP."</select></td></tr>";
...
echo "<td valign='top'><select id='DateActivite' name='DateActivite'>".$listeJours."</select><script>selectList('DateActivite','".date('d.m.Y')."')</script></td>";
if(empty($heures)) {
	$heures = "0.0";
}
