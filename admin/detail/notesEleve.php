...
if(isset($_POST['actionNote']) && !empty($_POST['actionNote'])) {
	...
	if($_POST['actionNote']=='update') {
		// maj: r�cup�ration de l'id et des attributs
		$IDNote = $_POST['idNote'];
		...
		if(!empty($_POST['Note'.$IDNote])) {
			...
			// modification de la remarque g�n�rale
		...
		if(mysqli_num_rows($resultat)==0) {
			// l'entr�e avec IDTypeNote=0 n'existe pas pour stocker la pond�ration, on l'ajoute
			...
			// mise � jour de la pond�ration pour le th�me (ligne avec IDTypeNote =0)
		...
		// ajout: r�cup�ration des donn�es de base et construction de l'identifiant unique
		$annee = $_POST['anneeNote'];
		$semestre = $_POST['semestreNote'];
		$theme = $_POST['themeNote'];
		if(empty($theme)) {
			// cr�ation de th�me ou nouvelle remarque
			...
			if($idTypeNote==1) {
				// TE -> ponderation par d�faut: 1
				$ponderation = $_POST['Ponderation'.$idChamp];
				if(empty($ponderation)) {
					$ponderation = 1;
				}
			} else {
				// on laisse � z�ro -> utiliser la pond�ration du th�me
				$ponderation = 0;
				// pour les notes autres que TE, tester si pas d�j� existante pour le th�me
				...
				if($resultat[0]!=0) {
					// la note existe d�j�
					$messageErreur = "La note existe d�j�!";
				}
...
echo "<tr newSemestre='1' style='display:none'><td colspan='9'>Nouvelle p�riode: <br><select name='anneeNew'>".$optionAnnee."</select>\n";
...
function clotureTheme() {
	...
	// ajouter ligne r�cap pour affichage sans d�tail
	$iconSuppr = "";
	if($cntNoteTheme==0) {
		// theme vide -> possibilit� de le supprimer
		$iconSuppr = "<img src='/iconsFam/cross.png' align='absmiddle' onmouseover=\"Tip('Supprimer le th�me')\" onmouseout='UnTip()' onclick='submitDeleteTheme(\"".$theme."\", \"".$annee."\", \"".$semestre."\")'> ";
	}
	if($theme!=0 && $theme>=10) {	// ne pas afficher les lignes fictives
		...
		if($cntNoteTheme!=0&&$moyTheme!=0&&$modeEvaluation=="theme") {
			echo "\n<tr block$annee$semestre$theme='1' style='display:".$displayExpand."'><td width='15'></td><td width='250'><i>Moyenne Th�me</i></td><td width='40'></td><td width='40' align='center'>".$moyTheme."</td><td width='660' colspan='5'></td></tr>";
		}
...
function clotureSemestre() {
	...
	if(!empty($libellePeriode)  && (array_sum($cntNotesComp)!=0) ) {
		...
		$ligneMoy .= "<tr><td colspan='3' width='305'><i>Moyenne calcul�e ".$libellePeriode."</i></td>";
		$cntNoteTot = 0;
		foreach ($compAbbr as $key => $value) {
		...
		*/
		...
		if(empty($remGen)) {
			// si aucune remarque pr�sent, ajouter les champs de cr�ation
			$remGen = "<tr onclick='toggleEdit(\"tdRem$annee$semestre\");'><td tdRem$annee$semestre='1' id='hidethis' style='display:none' colspan='3' valign='top' width=305><b>Remarque sur la p�riode</b></td>";
		...
		if(!empty($histo)&&$annee!=0) {
			echo "<tr><td colspan='3'><b>Historique p�riode</b></td><td colspan='6'>".$histo."</td></tr>";
		}
		// ligne d'espacement
...
while ($ligne = mysqli_fetch_assoc($resultat)) {
	if($ligne['Annee'] != $annee || $ligne['NoSemestre'] != $semestre) {
		...
		switch($ligne['NoSemestre']) {
			case 1: $libellePeriode = "1er semestre";
					break;
			case 2: $libellePeriode = "2�me semestre";
					break;
		}
		$annee = $ligne['Annee'];
		...
		*/
		...
		echo "<tr><td colspan='2' width='265'><font size='2' face='Verdana'><nobr><b>Ann�e ".$ligne['Annee']."/".($ligne['Annee']+1)." - ".$libellePeriode."</b></nobr></font></td><td width='40'></td><td width='40'></td><td colspan='5' width='660' align='right'>";
		if($modeEvaluation=="theme") {
			echo "<img src='/iconsFam/folder_add.png' onmouseover=\"Tip('Ajouter un th�me pour la p�riode')\" onmouseout='UnTip()' onclick='limitEvent(event);toggle(\"newTheme$annee$semestre\");'>";
		}
		echo "&nbsp;<img src='/iconsFam/comment_add.png' onmouseover=\"Tip('Ajouter une remaque sur la p�riode')\" onmouseout='UnTip()' onclick='toggleEdit(\"tdRem$annee$semestre\");'>";
		//echo "&nbsp;<img src='/iconsFam/page_add.png' onmouseover=\"Tip('Ajouter le bulletin de la p�riode')\" onmouseout='UnTip()' onclick='toggleEdit(\"tdBul$annee$semestre\");'>";

		if($ligne['IDTheme']<10 && $ligne['IDTypeNote']==0) { // il ne reste que l'ann�e -> on peut effacer la ligne
			echo "&nbsp;<img src='/iconsFam/cross.png' onmouseover=\"Tip('Supprimer la p�riode')\" onmouseout='UnTip()' onclick='submitDeletePeriode(\"".$annee."\", \"".$semestre."\")'>";
		}
		echo "</td></tr>";
		...
		echo "<td width='40' align='center'><input type='text' size='1' name='PonderationTheme$annee$semestre' value='' onmouseover=\"Tip('Pond�ration du th�me, laisser vide pour utiliser la pond�ration de base de ce th�me')\" onmouseout='UnTip()'></td>";
	...
	if($ligne['IDTheme']>=10 && $ligne['IDTheme']!=$theme) {
		...
		// recherche du nombre de semaine selon journaux pour pond�ration automatique
		...
		echo "<td width='40' align='center' pond$annee$semestre$theme='1' onclick='toggleTD(\"pond$annee$semestre$theme\");limitEvent(event);' onmouseover=\"Tip('Pond�ration calcul�e: ".$pondAuto."')\" onmouseout='UnTip()'><b>Pond</b><br>".$pondTheme."</td>";
		...
		echo "<td width='40' align='center'><b>Note</b><br>&nbsp;</td><td colspan='5' align='right' width='660'><img src='/iconsFam/script_add.png' onmouseover=\"Tip('Ajouter une note au th�me')\" onmouseout='UnTip()' onclick='limitEvent(event);toggle(\"new$annee$semestre$theme\",\"focus$annee$semestre$theme\");'></td></tr>";
		// ajouter une ligne d'insertion
		$optionTypeNote = "<option value='1'>TE</option>"; // construction de la liste d�roulante pour ajout Note
	...
	if($ligne['IDTypeNote']>1 && $ligne['IDTypeNote']<6 ) { //&& empty($ligne['Note'])) {
		// recherche des moyennes des �valuations du th�me concern�
	...
	switch($ligne['IDTypeNote']) {
		...
		case 2:
			...
			if(!empty($ligne['Note'])) {
				if(!empty($moyEstimee)) {
					echo " onmouseover=\"Tip('Moyenne des �valuations: ".$moyEstimee.", yc non valid�es: ".$moyEstimeeNoValid."')\" onmouseout='UnTip()'";
				}
		...
		case 6:
			...
			$remGen .= "<tr onclick='toggleEdit(\"tdRem$annee$semestre\");'><td colspan='3' valign='top' width=305><b>Remarque sur la p�riode</b></td>";
