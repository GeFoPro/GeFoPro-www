...
$prpond = -1;
$dipond = -1;
// modification ou ajout d'une �valuation
if(isset($_POST['action'])&&!empty($_POST['action'])) {
	...
	if(!isset($trpond) || !isset($prpond) || !isset($dipond) || empty($trtxt) || empty($prtxt) || empty($ditxt) || empty($amtxt) || empty($notete) || empty($noteap) || empty($notere)  || empty($notese)) {
		//$msg="<font color='#FF0000'>Tous les champs doivent �tre renseign�s</font><script>alert(\"<font color='#FF0000'>Tous les champs doivent �tre renseign�s!</font>\");</script>";
		$msg="<script>alert(\"Tous les champs doivent �tre renseign�s!\");</script>";
		if(!isset($trpond)) {
			$trpond = -1;
		}
		if(!isset($prpond)) {
		...
		if($action=='Modifier') {
			...
			//echo $requete;
    			$resultat =  mysql_query($requete);
			$msg = "<font color='#088A08'>Evaluation modifi�e avec succ�s</font>";
		} else {
			$requete = "INSERT INTO evaluation (IDTheme, IDEleve, TravailPond, TravailRemarque, ProgretsPond, ProgretsRemarque, DifficultesPond, DifficultesRemarque, Ameliorations, NoteTechnique, NoteApplication, NoteRendement, NoteSavoirEtre) values ($IDTheme, $IDEleve, $trpond, \"$trtxt\", $prpond, \"$prtxt\", $dipond, \"$ditxt\", \"$amtxt\", $notete, $noteap, $notere, $notese)";
	    		//echo $requete;
			$resultat =  mysql_query($requete);
			$msg = "<font color='#088A08'>Evaluation ajout�e</font>";
		}
	}
}
// test de la p�riode pour notes
...
if(!$resultat || mysql_num_rows($resultat)==0) {
	$msgNote1 = "La p�riode ainsi que le th�me n'existent pas dans la gestion des notes. Les cr�er avec la pond�ration de base?";
} else {
	// test du th�me
	...
	if(mysql_num_rows($resultat)==0) {
		$msgNote2 = "Le th�me n'existe pas dans la gestion des notes. Le cr�er avec la pond�ration de base?";
	}
}
if(isset($_POST['validation'])&&!empty($_POST['validation'])) {
	...
	if($_POST['validation']=="lock") {
		...
		//echo $requete;
		mysql_query($requete);
		// ajouts n�cessaire � la saisie des notes
		if(!empty($msgNote1) ) {
			// cr�er la p�riode
			...
			mysql_query($requete);
			$msg = "<font color='#088A08'>P�riode ajout�e dans la gestion des notes</font>";
		}
...
echo "</h2></td>";
echo "<td width='200'><!-- h2>Auto-�valuation";
...
echo "<tr><td></td><td></td><td width='150' align='right'><b>Heures consacr�es:</b></td><td>".sprintf("%2.1f",$ligne['heures'])."h / ".sprintf("%d",$ligne['jours'])." jours</td></tr>";
echo "</table><br>";
...
if($IDTheme!=0) {
	...
	//echo "Pond�ration travail: ".$trpond;
	echo "<br><div id='corners'>";
	echo "<div id='legend'>Auto-�valuation du th�me/projet</div>";
	...
	echo "<tr><td><b>J'ai progress� et appris de nouvelles choses:</b></td><td width='70' align='right'>pas du tout</td>".ponderation('progrets',$prpond,$action) ."<td>�norm�ment</td></tr>";
	...
	echo "<tr><td colspan='8' height='30'></td></tr>";
	echo "<tr><td><b>J'ai rencontr� des difficult�s:</b></td><td width='70' align='right'>aucune</td>".ponderation('difficultes',$dipond,$action) ."<td>constamment</td></tr>";
	...
	echo "<tr><td valign='top'><b>Ce que je dois encore am�liorer:</b></td><td colspan='7'><textarea name='remarqueAmelioration' COLS=60 ROWS=4 ".$styleborder.">".$amtxt."</textarea></td></tr>";
	echo "<tr><td colspan='8' height='30'></td></tr>";
	echo "<tr><td><b>J'�value mon travail avec les notes de:</b></td><td width='70'>";
	...
	echo "<input type='texte' name='application' value='".$noteap."' size='3' ".$styleborder."></td><td colspan='6'>Application <img src='/iconsFam/information.png' align='absmiddle' onmouseover=\"Tip('Ai-je �t� capable d\'appliquer ce que je connais?<br>Ai-je mis tout en oeuvre pour parvenir � terminer les exercices, les montages ou le projet?<br>Est-ce complet?<br>Suis-je all� au bout des choses?')\" onmouseout='UnTip()'></td></tr>";
	...
	echo "<input type='texte' name='rendement' value='".$notere."' size='3' ".$styleborder."></td><td colspan='6'>Rendement <img src='/iconsFam/information.png' align='absmiddle' onmouseover=\"Tip('Ai-je utilis� le temps � ma disposition de mani�re efficace?<br>Est-ce que j\'ai pu termin� mon travail dans le temps imparti?')\" onmouseout='UnTip()'></td></tr>";
	echo "<tr><td></td><td width='70'>";
	$styleborder=""; if(!empty($action)&&(empty($notese)||!is_numeric($notese) || $notese>6)) $styleborder="style='border-color: #ff0000'";
	echo "<input type='texte' name='savoirEtre' value='".$notese."' size='3' ".$styleborder."></td><td colspan='5'>Savoir-�tre <img src='/iconsFam/information.png' align='absmiddle' onmouseover=\"Tip('Me suis-je comport� en professionnel?<br>Suis-je capable de rester concentr� sur mon travail?<br>Peut-on compter sur moi pour la r�alisation de ce qui m\'est demand� de faire?<br>Suis-je en ordre avec mes horaires, journaux, etc.?')\" onmouseout='UnTip()'></td>";
	echo "<td align='right'>";
	if(!hasAdminRigth()) {
		if($num_rows==0) {
			...
			if(empty($dateValidation) || $dateValidation=="0000-00-00") {
				...
				echo "<i>Document valid� le ".date('d.m.Y', strtotime($dateValidation))."</i>";
			}
		}
	}
	echo "</td></tr>";
	...
	if(hasAdminRigth()&&$num_rows==1) {
		if(empty($anneeEval) || $anneeEval=="0000-00-00") {
			...
			$optionAnnee .= "</select><select name='semestreNew'><option value='1'>1er semestre</option><option value='2'>2�me semestre</option></select>";
		} else {
			$optionAnnee = $anneeEval."/".($anneeEval+1).", semestre ".$semestreEval;
		}

		if(empty($dateValidation) || $dateValidation=="0000-00-00") {
			$valTxt = "ouverte </td><td><img src='/iconsFam/page_go.png' onmouseover=\"Tip('Valider l\'auto-�valuation')\" onmouseout='UnTip()' align='absmiddle' onClick='submitValidation(\"lock\")'>";
		} else {
			$valTxt = "valid�e le ".date('d.m.Y', strtotime($dateValidation))." </td><td><img src='/iconsFam/page_delete.png' onmouseover=\"Tip('D�valider l\'auto-�valuation')\" onmouseout='UnTip()' align='absmiddle' onClick='submitValidation(\"unlock\")'>";
		}
		...
		echo "<td align='center' width='400'></td><td width='150' align='right'><b>Auto-�valuation:</b></td><td>".$valTxt."</td></tr>";
		echo "<tr><td width='200'></td><td align='center' width='400'></td><td width='150' align='right'><b>P�riode:</b></td><td>".$optionAnnee."</td></tr>";
		echo "</table>";
		...
		if(!empty($dateValidation) && $dateValidation!="0000-00-00") {
			...
			echo "<tr><td width='300'></td><td width='70'><input type='texte' name='savoirEtreP' value='".$notePse."' size='3'></td><td colspan='5'><b>Savoir-�tre</b><br> ".$remPse."</td></tr>";
			echo "</table><br>";
		}
		echo "</div>";
	}
