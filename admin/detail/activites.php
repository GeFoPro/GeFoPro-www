...
if(!isset($_POST['noSemaine']) || "" == $_POST['noSemaine']) {
	//echo "pas trouv� en POST: ".$_POST['noSemaine'];
	if(!isset($_SESSION['noSemaine']) || "" == $_SESSION['noSemaine']) {
		//echo "- pas trouv� en session: ".$_SESSION['noSemaine'];
		$noSemaine = date('W');
		$anneeCalc = date('Y');
	} else {
		//echo "- trouv� en session: ".$_SESSION['noSemaine'];
		$noSemaine = $_SESSION['noSemaine'];
		$anneeCalc = $_SESSION['anneeCalc'];
	}
} else {
	//echo "trouv� en POST: ".$_POST['noSemaine'];
...
if(isset($_GET['anneeAff'])) {
	// affichage forc� des activit�s de l'ann�e enti�re (semaine 31 = d�but ann�e scolaire), sp�cifi�e en param�tre GET
	$noSemaine = 31;
...
$vendredi = $lundi + 86400*5 - 3600*2; // retrait de 2h -> vendredi soir m�me si GMT +1 ou +2
...
//$triPeriode = 1; // 1 = semster en cours, 2 = ann�e en cours, 0 = tout
$semestreAct = 1;
if($noSemaine<30) {
	// de janvier � juillet
	// calcul du lundi du 2�me semestre
	$lundisem2=$dateCalc-86400*($jour_semaine-1)+604800*($noSemaineSem2-1);
	if($noSemaine>=$noSemaineSem2) {
		// uniquement 2�me semestre
		$betweenSQLSem = "between '".date("Y-m-d",$lundisem2)."' and '".($anneeCalc)."-07-31'";
		$semestreAct = 2;
	} else {
		// fin du premier semestre en d�but d'ann�e civile
...
if(isset($_POST['AjoutActivite'])) {
	...
	if(empty($_POST['DateActivite'])) {
		...
		$dateCheck = strtotime($_POST['DateActivite'])+3600; // evite les probl�me de calcul avec l'heure d'�t�
		if($dateCheck<$lundi || $dateCheck>$vendredi) {
			$msgErreur = "<font color='#FF0000'>La date saisie (".date("d.m.Y",strtotime($_POST['DateActivite'])).") se trouve en dehors de la semaine concern�e!</font>";
		}
	}
	//echo $dateCheck."-".$lundi."-".$vendredi;
	if($heures=="0.0"&&empty($heureDebut)&&empty($heureFin)) {
		// aucune info sur le temps/dur�e -> on utilise le mode chrono avec l'heure actuelle
		$heureDebut = $heureAct;
	} else {
		if($heures!="0.0") {
			// si dur�e renseign�e, on s'assure que les heures de d�but et de fin ne sont pas enregistr�es
			$heureDebut = "00:00";
			$heureFin = "00:00";
		} else {
			if(!empty($heureDebut)) {
				...
				if(empty($time1)) {
					$msgErreur .= "<font color='#FF0000'>L'heure de d�but n'est pas correcte!</font>";
				}
				if(!empty($heureFin)) {
					// heures de d�but et de fin sp�cifi�e -> calcul de la dur�e
				...
				$msgErreur .= "<font color='#FF0000'>L'heure de d�but est obligatoire!</font>";
			}
		}
	}
...
if(isset($_POST['activite'])&&!empty($_POST['activite'])) {
	...
	//echo $dateCheck."-".$lundi."-".$vendredi;
	if($dateCheck<$lundi || $dateCheck>$vendredi) {
		$msgErreur = "<font color='#FF0000'>La date saisie (".date("d.m.Y",strtotime($_POST['DateJournal'.$activite])).") se trouve en dehors de la semaine concern�e!</font>";
	}
	if($heuresMaj=="0.0"||(empty($heuresMaj)&&empty($heureDebutMaj)&&empty($heureFinMaj))) {
		// aucune info sur le temps/dur�e -> maj impossible
		$msgErreur .= "<font color='#FF0000'>Aucune heure ou dur�e sp�cifi�e!</font>";
	} else {
		if(!empty($heureDebutMaj)) {
			...
			if(empty($heuresMaj)) {
				// mode chrono sans heure de d�but
				$msgErreur .= "<font color='#FF0000'>L'heure de d�but est obligatoire!</font>";
			} else {
				// mode dur�e
				$heureDebutMaj = "00:00";
...
if(isset($_POST['validation'])&&!empty($_POST['validation'])) {
	...
	if($_POST['validation']=="lock") {
		...
		mysqli_query($connexionDB,$requete);
		if($vue==1) {
			// si vue semaine, valider journaux de la semaine �galement
			$requete = "update journal set DateValidation = \"".date('Y-m-d')."\" where IDEleve = $IDEleve and DateValidation is null and (DateJournal between '".date('Y-m-d', $lundi)."' and '".date('Y-m-d', $vendredi)."')";
			//echo $requete;
			mysqli_query($connexionDB,$requete);
		} else {
			// si vue theme, valider tous les journaux jusqu'� la semaine pr�c�dente
...
if(isset($_POST['evaluation'])&&!empty($_POST['evaluation'])) {
	...
	//echo "<br>comp�tence: ".$compSel;
	//echo "<br>type: ".$typeSel;
	//echo "<br>�valuation: ".$_POST['eval'.$_POST['evaluation']];
...
//echo "<center><font color='#088A08'>Nouveaut�: Les erreurs de saisie sont affich�es dans la semaine concern�e.</font></center>";
...
if($vue==1) {
	//echo "Tri: <select name='tri'><option value='1'>Semaine</option><option value='2'>Mois</option><option value='3'>Ann�e</option><option value='4'>Tous</option></select>";
	echo "Vue: <select name='vue' onChange='document.getElementById(\"myForm\").submit();'><option value='1'>Semaine</option><option value='2'>Th�me</option></select>";
} else {
	echo "Vue: <select name='vue' onChange='document.getElementById(\"myForm\").submit();'><option value='1'>Semaine</option><option value='2' selected>Th�me</option></select>";
}
echo "</td></tr>";

// recherche du theme en cours selon journal de l'apprenti si pas donn� en param�tre, pour la vue th�me et si pas donn� en param�tre
$lastIDTheme;
...
if($vue==1) {
	...
	echo " P�riode: <select name='triPeriode' onChange='document.getElementById(\"myForm\").submit();'>";
	$txtSelected = " selected";
	echo "<option value='1'".($triPeriode==1?$txtSelected:"").">Semestre en cours</option>";
	echo "<option value='2'".($triPeriode==2?$txtSelected:"").">Ann�e en cours</option>";
	// recherche derni�re date d'�valuation pour le th�me donn�
	...
	if(!empty($ligne['noSemaine'])) {
		...
		echo "<option value='3'".($triPeriode==3?$txtSelected:"").">Non �valu�</option>";
	} else {
	...
	echo "Th�me: <select name='IDThemeSel' onChange='document.getElementById(\"myForm\").submit();'>";
	if(hasAdminRigth()) {
		echo "<option value='2'";
...
echo "<div id='legend' onClick='toggleTable(\"tblActivite\")'>Activit�s</div>";
if($vue==1) {
	if(!$triSemaine) {
		echo "<div id='criteres' width='100%' align='right'><i>Tri par th�me &nbsp;<a href='activites.php?from=journaux&idEleve=$IDEleve&nom=$nom&prenom=$prenom&Classe=$classe&triSem=1'><img src='/iconsFam/table_refresh.png' style='vertical-align: middle;' onmouseover=\"Tip('Changer de tri par jour')\" onmouseout='UnTip()'></a></i></div>";
	} else {
		echo "<div id='criteres' width='100%' align='right'><i>Tri par jour &nbsp;<a href='activites.php?from=journaux&idEleve=$IDEleve&nom=$nom&prenom=$prenom&Classe=$classe&triSem=0'><img src='/iconsFam/table_refresh.png' style='vertical-align: middle;' onmouseover=\"Tip('Changer de tri par th�me')\" onmouseout='UnTip()'></a></i></div>";
	}
} else {
...
//$triSemaine = 0; // 0 = th�me, 1 = jour
// requete principale
if(empty($IDQuery)) {
	// requete pour la semaine donn�e
	...
	switch($triPeriode) {
		...
		case 3: $requete .= " and (DateJournal >= '".date("Y-m-d",$maxDateEval)."')"; echo "<div id='criteres' width='100%' align='right'><i>P�riode d�s le ".date("d.m.Y",$maxDateEval).", semaine ".date("W",$maxDateEval)."</i></div>"; break;
	}
	if($IDQuery==2) {
...
if(!$triSemaine) {
	echo "<tr><th width='105'>Th�me</th><th width='90' align='center' onClick=\"location.href='activites.php?from=journaux&idEleve=$IDEleve&nom=$nom&prenom=$prenom&Classe=$classe&triSem=1'\">Date</th><th width='85' align='center'>Heures</th><th>Activit�s</th><th width='25' align='right'>";
} else {
	echo "<tr><th width='35'>Date</th><th width='250' onClick=\"location.href='activites.php?from=journaux&idEleve=$IDEleve&nom=$nom&prenom=$prenom&Classe=$classe&triSem=0'\">Th�me</th><th width='85' align='center'>Heures</th><th>Activit�s</th><th width='25' align='right'>";
}
if(empty($IDQuery)) {
	...
	if($IDQuery!=1 && $IDQuery!=2) {
		// ne pas proposer d'imprimmer si CPG selectionn�
...
while ($ligne = mysqli_fetch_assoc($resultat)) {
	...
	if(!$triSemaine) {
		if($last!=$ligne['IDTheme']) {
			if($last!=0) {
				...
				echo "<tr><td><i>Total th�me</i></td><td></td><td align='center'>".$total."h</td><td>";
				if($total!=$ligneHeures['heures']) {
					echo "(cumul sur l'ann�e scolaire: ".$ligneHeures['heures']."h";
					//if($objectif!=0) {
					//	echo "/".$objectif."h";
					//}
					echo ")";
						...
						while ($ligneRem = mysqli_fetch_assoc($resultatRem)) {
							...
							if(!empty($txtrem)) {
								if($typerem<=1&&hasAdminRigth()) {
									...
									echo "<tr><td valign='top'><font color='#FF7F00'>A v�rifier</font></td><td valign='top'></td><td align='center'></td><td><font color='#FF7F00'>".wiki2html($txtrem)."</font></td><td align='center' valign='top'><a href='activites.php?idEleve=$IDEleve&nom=$nom&prenom=$prenom&IDRemSuivi=$ligneRem[IDRemSuivi]&action=delete'><img src='/iconsFam/tick.png' align='absmiddle' onmouseover=\"Tip('Cl�turer l\'erreur')\" onmouseout='UnTip()' onclick='limitEvent(event)'></a></td></tr>";
								}
							}
			...
			// recherche du nombre d'heures d�j� effectu�es pour le th�me sur l'ann�e scolaire enti�re
			...
			if($ligne['Objectif']!=0) {
				echo "<b onmouseover=\"Tip('Objectif du th�me')\" onmouseout='UnTip()'>".$ligne['Objectif']."h</b>";
			}
	...
	if(!$triSemaine) {
		// on inscrit rien sous th�me (donn�e dans le groupe)
		echo "<td></td>";
	} else {
		// vue par jour, on inscrit le th�me sur chaque ligne
	...
	if($ligne['HeureDebut']!='00:00:00') {
		...
		if($nbrJour[$lastJour]>9) {
			...
			$msgErreur .= "Heures journali�res > 9h<br>";
	...
	if($ligne['DateValidation']!=0) {
		...
		if(!hasAdminRigth()) {
			echo "<img src='/iconsFam/lock.png' align='absmiddle' onmouseover=\"Tip('Saisie valid�e le ".date('d.m.Y', strtotime($ligne['DateValidation']))."')\" onmouseout='UnTip()'>";
		} else {
			echo "<img src='/iconsFam/bullet_green.png' align='absmiddle' onmouseover=\"Tip('Saisie valid�e le ".date('d.m.Y', strtotime($ligne['DateValidation']))."')\" onmouseout='UnTip()'>";
		}
	} else {
		$cntunlock++;
		if(!hasAdminRigth()) {
			if(empty($IDQuery)) {
				if($ligne['Heures']=='0.0') {
					echo "<img src='/iconsFam/clock_stop.png' align='absmiddle' onmouseover=\"Tip('Terminer l'activit�')\" onmouseout='UnTip()' onclick='limitEvent(event);stopActivite(\"$idJournal\",\"$heureAct\")'> ";
				}
			...
			if($ligne['Heures']=='0.0') {
				echo "<img src='/iconsFam/error.png' align='absmiddle' onmouseover=\"Tip('Activit� non termin�e')\" onmouseout='UnTip()'> ";
			}
			echo "<a href='activites.php?from=journaux&idEleve=$IDEleve&nom=$nom&prenom=$prenom&Classe=$classe&DateJournal=$ligne[DateJournal]'><img src='/iconsFam/bullet_red.png' align='absmiddle' onmouseover=\"Tip('Saisie � valider')\" onmouseout='UnTip()'></a>";
	...
	if($ligne['HeureDebut']!='00:00:00') {
		...
		if(empty($heureFinStr)) {
			echo "<img src='/iconsFam/clock_stop.png' align='absmiddle' onmouseover=\"Tip('Terminer l'activit�')\" onmouseout='UnTip()' onclick='limitEvent(event);setHeureFin(\"$idJournal\",\"$heureAct\")'>";
		} else {
...
if ($cnt==0) {
	...
	if(!$triSemaine) {
		if(empty($IDQuery) || $IDQuery==2) {
			...
			echo "</td><td align='center'>";
			//echo "<tr><td><i>Total th�me</i></td><td></td><td align='center'>".$total."h</td><td></td><td  align='center'>";
			/*
			if(hasAdminRigth()&&empty($IDQuery)) {
	...
	/*
		...
		while ($ligneRem = mysqli_fetch_assoc($resultatRem)) {
			...
			if(!empty($txtrem)) {
				if($typerem<=1&&hasAdminRigth()) {
					echo "<tr remarque$idRem='1' onclick='toggle(\"remarque$idRem\")'><td valign='top'><i>Suivi th�me</i></td><td valign='top'></td><td align='center'></td><td><i>".wiki2html($txtrem)."</i></td><td align='center' valign='top'>";
					if(empty($IDQuery)) {
						echo "<a href='activites.php?idEleve=$IDEleve&nom=$nom&prenom=$prenom&IDRemSuivi=$ligneRem[IDRemSuivi]&action=delete'><img src='/iconsFam/table_row_delete.png' align='absmiddle' onmouseover=\"Tip('Supprimer cette remarque')\" onmouseout='UnTip()' onclick='limitEvent(event)'></a>";
					}
					echo "</td></tr>";
	...
	*/
	...
	if(empty($IDQuery)) {
			...
			if($joursATE[$classe] != count($nbrJour)) {
				$msgErreur .= "Semaine incompl�te<br>";
			}
	}
	if(empty($classe)) {
		$msgErreur = "";
...
if(hasAdminRigth()) {
	if(empty($IDQuery)) {
		...
		if($cntlock!=0) {
			echo "<img src='/iconsFam/page_delete.png' onmouseover=\"Tip('D�valider le journal')\" onmouseout='UnTip()' align='absmiddle' onClick='submitValidation(\"unlock\",".$noSemaine.",".$anneeCalc.")'> ";
		}
	}
} else {
	if(($cntlock==0 || $cntunlock!=0) && empty($IDQuery)) {
		echo "<img src='/iconsFam/add.png' onmouseover=\"Tip('Nouvelle entr�e')\" onmouseout='UnTip()' onclick='toggle(\"newActivite\");document.getElementById(\"myForm\").HeuresActivite.select();' align='absmiddle'>";
	}
}
echo "</td></tr>";
echo "<tr newActivite='1' style='display:none'><td colspan='5' valign='bottom' height='30'><b>Nouvelle entr�e dans le journal:<b></td></tr>";
...
echo "</table></div><br>";
//if(!hasAdminRigth()) {
//	echo "<center> <b>Respectez les <u><a href='/wiki/index.php/Saisie_du_journal_de_travail'>consignes</a></u> pour la saisie de vos activit�s!</b></center>";
//}
echo "<br>";
...
$requeteTri = "";
	if(empty($IDQuery)) {
		...
		// recherche dates min max dans les journaux du th�me si vue theme
		if($vue==2) {
			if($IDQuery!=2) {
				// tous les th�mes
			...
			//}
				...
				//echo $requete;
				if(!empty($resultat)&&mysqli_num_rows($resultat)>0) {
					// des activit�s existent dans les journaux
					$ligne = mysqli_fetch_assoc($resultat);
					$max = $ligne['max'];
					$min = $ligne['min'];
					// -> calcul du premier lundi au dernier vendredi pour effectuer une recherche sur des semaines compl�tes
					if(!empty($min)) {
		...
		if($IDQuery!=2) {
			...
			$requeteTri .=  "th.IDTheme<>2"; // workaround pour que la requete fonction avec le AND de la p�riode ci-apr�s
		}
		switch($triPeriode) {
	...
	if(hasAdminRigth()) {
		...
		if(!empty($IDQuery)) {
			if(!empty($lundiMin)) {
				echo "<div id='criteres' width='100%' align='right'><i>P�riode CPG/ADM du lundi ".date('d.m.y', strtotime($lundiMin))." au vendredi ".date('d.m.y',strtotime($vendrediMax))."</i></div>";
			} else {
				if($IDQuery!=2) {
			...
			//echo "<div id='criteres' width='100%' align='right'><i>P�riode du lundi ".date('d.m.y', $lundi)." au vendredi ".date('d.m.y',$vendredi)."</i></div>";
		}
			...
			//if(!empty($txtrem)) {
				switch($typerem) {
					...
					case 2:
						...
						break;
					case 3:
						echo "<tr><td valign='top'><font color='#FF7F00'><i>JRN</i> - A v�rifier</font></td><td><font color='#FF7F00'>".$daterem."</font></td><td align='center' valign='top'><font color='#FF7F00'> ".$auteur." </font></td><td><font color='#FF7F00'>".wiki2html($txtrem)."</font></td><td align='center' valign='top'><a href='activites.php?idEleve=$IDEleve&nom=$nom&prenom=$prenom&IDRemSuivi=$ligneRem[IDRemSuivi]&action=delete'><img src='/iconsFam/tick.png' align='absmiddle' onmouseover=\"Tip('Cl�turer l\'erreur')\" onmouseout='UnTip()' onclick='limitEvent(event)'></a></td></tr>";
						break;
					case 0:
						if(hasAdminRigth()) {
		...
		if(empty($IDQuery)) {
			if(hasAdminRigth()) {
				...
				if (mysqli_num_rows($resultatRem) != 0) {
					...
					//echo "<tr><td></td><td></td><td></td><td><b><font color='#FF0000'>Des erreurs de saisies sur d'autres semaines doivent encore �tre corrig�es!</font></b></td><td></td></tr>";
					echo "<tr><td align='center'><b><font color='#FF0000'>Des erreurs de saisies sur d'autres semaines doivent encore �tre corrig�es!</font></b></td></tr>";
				}
			}
		}
	}

	// ajout remarques g�n�rale de l'�l�ve
	/*
	if(hasAdminRigth()) {
	...
	*/
	...
	if(hasAdminRigth()) {
		...
		// IDEtatProjet=1 and, supprim� le 12.06.2015
		$resultat =  mysqli_query($connexionDB,$requete);
		$option = "";
		//while ($ligne = mysqli_fetch_assoc($resultat)) {
...
if(!empty($typeEvaluation) && $IDQuery!=2) {
	if(($vue==1 && $modeEvaluation=="hebdo") || ($vue==2 && $modeEvaluation=="theme" && $IDQuery!=1)) {
		...
		if($vue==1) {
			...
			echo "<div id='legend' onClick='toggleTable(\"tblEval\")'>Evaluation du th�me</div>";
			...
			// vue par th�me -> recherche initale de la liste des th�mes valid�s pour l'�l�ve, le th�me donn�
			...
			if(!empty($resultat)&&$numEval>0) {
				echo "<div id='criteres' width='100%' align='right'><i>Evaluation(s) pr�sente(s), semaine(s) no: ";
				while ($ligne = mysqli_fetch_assoc($resultat)) {
					$evalCnt++;
					echo "<a id='circle";
				...
				// noSemaine pas existante dans la liste ou derni�re �val valid�e, on en ajoute une nouvelle
				if(!$evalAct) {
					$noSemaine = date("W");
		...
		if($typeEvaluation=='abcd') {
			echo "<tr><th width='175'>Comp�tences</th><th width='50'></th><th width='10' align='center'>A</th><th width='10' align='center'>B</th><th width='10' align='center'>C</th><th width='10' align='center'>D</th><th width='10'></th><th>Remarques</th><th width='50' align='center'>Moyenne</th><th width='10'></th></tr>";
		} else {
			echo "<tr><th width='175'>Comp�tences</th><th width='50'></th><th width='40' align='center'>Note</th><th width='10'></th><th>Remarques</th><th width='50' align='center'>Moyenne</th><th width='10'></th></tr>";
		}

		// rechercher des �valuation
		if($vue==1) {
			// vue par semaine -> recherche par noSemaine/ann�e
		...
		foreach ($compEval as $key => $value) {
			...
			if($key==$ligne['IDCompetence']) {
				// competence trouv�e dans les donn�es DB
				if(1==$ligne['IDTypeEval']) {
					// competence pour APP trouv�e dans DB
					$noteAPP = $ligne['Note'];
					if(!empty($noteAPP)) {
						$nbrNotes++;
					}
					...
					//	$validation = $ligne['DateValidation'];
					//}
					// test si competence MAI �galement pr�sente
					$ligne = mysqli_fetch_assoc($resultat);
					if($key==$ligne['IDCompetence'] && 2==$ligne['IDTypeEval']) {
						// competence MAI �galement pr�sente
						$noteMAI = $ligne['Note'];
						if(!empty($noteMAI)) {
							$nbrNotes++;
						}
						...
						// prochain enregistrement � r�cup�rer
						$ligne = null;
					} else {
						// aucune comp�tence trouv�e ni pour APP, ni pour MAI -> on garde la ligne actuelle pour le prochain passage
					}
				} else {
			...
			//echo "<tr onclick='".$onClickAPP."(\"tdid3\")'><td valign='top'>Comp�tences m�thodologiques</td><td valign='top'><i>APP</i></td><td valign='top'><input type='radio' name='Aam' value='' ".$radioAPP." onclick='limitEvent(event)'></td><td valign='top'><input type='radio' name='A' value='' ".$radioAPP." onclick='limitEvent(event)'></td><td valign='top'><input type='radio' name='A' value='' ".$radioAPP." onclick='limitEvent(event)'></td><td valign='top'><input type='radio' name='A' value='' ".$radioAPP." onclick='limitEvent(event)'></td><td></td><td  tdid3='1' valign='top' style='display:none'><textarea name='compProfAPP' COLS=40 ROWS=4></textarea></td><td tdid3='1'></td></tr>";
			...
			//echo "<tr onclick='".$onClickAPP."(\"tdid5\")'><td valign='top'>Comp�tences sociales</td><td valign='top'><i>APP</i></td><td valign='top'><input type='radio' name='Aas' value='' ".$radioAPP." onclick='limitEvent(event)'></td><td valign='top'><input type='radio' name='A' value='' ".$radioAPP." onclick='limitEvent(event)'></td><td valign='top'><input type='radio' name='A' value='' ".$radioAPP." onclick='limitEvent(event)'></td><td valign='top'><input type='radio' name='A' value='' ".$radioAPP." onclick='limitEvent(event)'></td><td></td><td  tdid5='1' valign='top' style='display:none'><textarea name='compProfAPP' COLS=40 ROWS=4></textarea></td><td tdid5='1'></td></tr>";
		...
		if(99==$ligne['IDCompetence']) {
			// observation trouv�e dans les donn�es DB
			if(1==$ligne['IDTypeEval']) {
				// observation pour APP trouv�e dans DB
				$obsAPP = $ligne['Remarque'];
				// test si observation MAI �galement pr�sente
				$ligne = mysqli_fetch_assoc($resultat);
				if(99==$ligne['IDCompetence'] && 2==$ligne['IDTypeEval']) {
					// observation MAI �galement pr�sente
					$obsMAI = $ligne['Remarque'];
					if($resp==null) {
						$resp = $ligne['Responsable'];
						$abbrResp = $ligne['abbr'];
				...
				// seule observation MAI pr�sente
				$obsMAI = $ligne['Remarque'];
				if($resp==null) {
					$resp = $ligne['Responsable'];
					$abbrResp = $ligne['abbr'];
		...
		if(hasAdminRigth() || !empty($validation)) {
			...
			if(!empty($validation)) {
				echo "<i>Evaluation valid�e le ".date('d.m.Y', strtotime($validation))." par ".$abbrResp."</i> ";
				if(hasAdminRigth() && $resp==$_SESSION['user_login']) {
					echo "<img src='/iconsFam/page_delete.png' onmouseover=\"Tip('D�valider l\'�valuation')\" onmouseout='UnTip()' align='absmiddle' onClick='submitValidation(\"unlockEval\",".$noSemaine.",".$anneeCalc.")'> ";
				}
				echo "<a href='impressionJournalPDF.php?IDEleve=".$IDEleve."&IDTheme=".$IDQuery."&annee=".$anneeCalc."&noSemaine=".$noSemaine."&nom=".$nom."&prenom=".$prenom."&tri=".$triPeriode."&only=eval' target='pdf'><img src='/iconsFam/page_white_acrobat.png' align='absmiddle' onmouseover=\"Tip('Imprimer l\'�valuation')\" onmouseout='UnTip()'></a>";
			} else {
				...
				if(hasAdminRigth() && $resp==$_SESSION['user_login']) { // && $nbrNotes==count($compEval)*2
					...
					echo "<img src='/iconsFam/page_go.png' onmouseover=\"Tip('Valider l\'�valuation')\" onmouseout='UnTip()' align='absmiddle' onClick='submitValidation(\"lockEval\",".$noSemaine.",".$anneeCalc.")'>";
				}
			}
			echo "</td></tr>";
		}
		...
		echo "<tr><td></td><td>Ai-je �t� capable de mettre en application ce que j'ai appris?</td></tr>";
		echo "<tr><td width='100' colspan='2'><b>Comp�tences m�thodologiques:</b></td></tr>";
		echo "<tr><td></td><td>Ai-je mis tout en oeuvre pour parvenir � terminer les exercices, les montages ou le projet?</td></tr>";
		echo "<tr><td></td><td>Est-ce complet et de qualit�? Suis-je all� au bout des choses?</td></tr>";
		echo "<tr><td></td><td>Ai-je utilis� le temps � ma disposition de mani�re efficace?</td></tr>";
		echo "<tr><td></td><td>Est-ce que j'ai pu terminer mon travail dans le temps imparti?</td></tr>";
		echo "<tr><td></td><td>Me suis-je organis� correctement?</td></tr>";
		echo "<tr><td width='100' colspan='2'><b>Comp�tences sociales:</b></td></tr>";
		echo "<tr><td></td><td>Ai-je accept� et appliqu� les d�cisions prises?</td></tr>";
		echo "<tr><td></td><td>Me suis-je comport� en professionnel?</td></tr>";
		echo "<tr><td></td><td>Suis-je capable de rester concentr� sur mon travail?</td></tr>";
		echo "<tr><td></td><td>Peut-on compter sur moi pour la r�alisation de ce qui m'est demand� de faire?</td></tr>";
		...
		*/
		...
		echo "</table><br>";
		if($typeEvaluation=='abcd') {
			echo "<b>Echelle d'�valuation:</b><br><table border=0>";
			echo "<tr><td width='20'><b>A</b></td><td>Exigences d�pass�es / tr�s bon, quantitativement et qualitativement</td></tr>";
			echo "<tr><td width='20'><b>B</b></td><td>Exigences atteintes / bon, r�pondant bien aux objectifs</td></tr>";
