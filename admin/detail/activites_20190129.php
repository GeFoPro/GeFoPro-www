...
if(isset($_POST['AjoutActivite'])) {
	...
	if(empty($_POST['DateActivite'])) {
		...
		$dateCheck = strtotime($_POST['DateActivite']);
	...
	if(empty($msgErreur)) {
		// ajout d'un attribut
   		$requete = "INSERT INTO journal (IDTheme, IDEleve, DateJournal, Heures, Commentaires) values ($IDTheme, $IDEl, \"$date\", \"$heures\", \"$commentaires\")";
...
if(isset($_POST['activite'])&&!empty($_POST['activite'])) {
	...
	$dateCheck = strtotime($_POST['DateJournal'.$activite]);
	if($dateCheck<$lundi || $dateCheck>$vendredi) {
		...
		$requete = "update journal set IDTheme=$themeMaj , DateJournal=\"".$dateMaj."\", Commentaires=\"".$commentairesMaj."\", Heures=$heuresMaj where IDJournal=".$activite;
		//echo $requete;
    		$resultat =  mysql_query($requete);
	}
...
if(isset($_POST['validation'])&&!empty($_POST['validation'])) {
	...
	if($_POST['validation']=="lock") {
		...
		if($vue==1) {
			...
			// si vue theme, valider tous les journaux jusqu'à cette semaine
			$requete = "update journal set DateValidation = \"".date('Y-m-d')."\" where IDEleve = $IDEleve and DateValidation is null and IDTheme=".$IDTheme." and DateJournal <= '".date('Y-m-d', $vendredi)."'";
			//echo $requete;
			mysql_query($requete);
		}
		...
		foreach($listeId as $key => $valeur) {
			if($valeur[0]==$IDEleve) {
				if($key!=0) {
					...
					echo "<img src='/iconsFam/resultset_previous.png'></a>";
				}
				echo $nom." ".$prenom;
				if($key<count($listeId)-1) {
					echo "<a href='activites.php?from=".$from."&nom=".$listeId[$key+1][1]."&prenom=".$listeId[$key+1][2]."&idEleve=".$listeId[$key+1][0]."&Classe=".urlencode($listeId[$key+1][3])."'>";
					echo "<img src='/iconsFam/resultset_next.png'></a>";
				}
...
if($vue==1) {
	echo "<h2><img src='/iconsFam/resultset_previous.png' onClick='submitSemaineAnnee(".$noSemMoinsUn.",".$anneeMoinsUn.")'>Semaine ".$noSemaine;
...
if($vue==1) {
	...
	echo "<tr><td colspan='3' align='center'>".$lundiTxt." <div id='circlesel' onmouseover=\"Tip('Semestre ".$semestreAct."')\" onmouseout='UnTip()'>".$semestreAct." </div></td></tr>";
} else {
	//$IDQuery = $IDThemeJrn;
	...
	if(hasAdminRigth()) {
		echo "<option value='1'>CPG - Remarque</option>";
	}
	echo $option;
	echo "</select>";
	//echo "</td></tr>";
...
echo "<tr><th width='175'>Thème</th><th width='90'>Date</th><th width='40'>Heures</th><th>Activités</th><th width='10' align='center'>";
if(empty($IDQuery)) {
	echo "<a href='impressionJournalPDF.php?IDEleve=".$IDEleve."&noSemaine=".$noSemaine."&annee=".$anneeCalc."&nom=".$nom."&prenom=".$prenom."' target='pdf'><img src='/iconsFam/page_white_acrobat.png' onmouseover=\"Tip('Imprimer le journal')\" onmouseout='UnTip()'></a>";
} else {
	echo "<a href='impressionJournalPDF.php?IDEleve=".$IDEleve."&IDTheme=".$IDQuery."&annee=".$anneeCalc."&noSemaine=".$noSemaine."&nom=".$nom."&prenom=".$prenom."&tri=".$triPeriode."' target='pdf'><img src='/iconsFam/page_white_acrobat.png' onmouseover=\"Tip('Imprimer le journal')\" onmouseout='UnTip()'></a>";
}
echo "</th></tr>";
$resultat =  mysql_query($requete);
$cnt=0;
...
while ($ligne = mysql_fetch_assoc($resultat)) {
	...
	//}
	if(hasAdminRigth()) {
		echo "<tr onClick=\"location.href='activites.php?from=journaux&idEleve=$IDEleve&nom=$nom&prenom=$prenom&Classe=$classe&DateJournal=$ligne[DateJournal]&vue=1'\"><td></td>";
	} else {	
		if($ligne['DateValidation']!=0) {
			echo "<tr journal$idJournal='1'><td></td>";
		} else {
			if(empty($IDQuery)) {
				echo "<tr journal$idJournal='1' onclick='toggle(\"journal$idJournal\")'><td></td>";
			} else {
	...
	//}
	echo date('d.m.Y', strtotime($ligne['DateJournal']))."</td><td valign='top' align='center'>";
	...
	echo "</td><td>".wiki2html($ligne['Commentaires'])."</td><td align='center' valign='top'>";
	if($ligne['DateValidation']!=0) {
		$cntlock++;
	...
	echo "<tr journal$idJournal='1' style='display:none' onclick='toggle(\"journal$idJournal\")'><td valign='top'><select id='IDTheme".$idJournal."' name='IDTheme".$idJournal."' onclick='limitEvent(event)'>".$optionAPP."</select></td>";
	...
	echo "<td valign='top'><select id='DateJournal".$idJournal."' name='DateJournal".$idJournal."' onclick='limitEvent(event)'>".$listeJours."</select><script>selectList('DateJournal".$idJournal."','".date('d.m.Y', strtotime($ligne['DateJournal']))."')</script></td>";
	...
	echo "<td><textarea name='Commentaires".$idJournal."' COLS=60 ROWS=20 onclick='limitEvent(event)'>".$ligne['Commentaires']."</textarea></td>";
	echo "<td valign='top'><img src='/iconsFam/tick.png' align='absmiddle' onmouseover=\"Tip('Enregister')\" onmouseout='UnTip()' onclick='submitActivite(\"$idJournal\")'></td></tr>";
...
if ($cnt==0) {
	...
	*/
	...
	echo "<tr><td><b><i>Total</i></b></td><td>";
...
echo "<tr newActivite='1' style='display:none'><td valign='top'><select name='IDTheme'>".$optionAPP."</select></td>";
//echo "<td valign='top'><input name='DateActivite' size='8' maxlength='10' value='".date('d.m.Y')."'></input></td>";
echo "<td valign='top'><select name='DateActivite'>".$listeJours."</select></td>";

if(empty($heures)) {
	$heures = "0.0";
}
echo "<td valign='top' align='center'><select name='HeuresActivite'>".$listeHeures."</select></td>";
echo "<td valign='top'><textarea name='RemarqueActivite' COLS=60 ROWS=20>".$commentaires."</textarea></td>";
...
// suivi
	...
	if(hasAdminRigth()) {
		$requeteRem .= " union (SELECT 0, Nom, Date, Remarque, 0, 'ALL', 0 FROM $tableAttribEleves el join $tableAttribut att on el.IDAttribut=att.IDAttribut where IDEleve = ".$IDEleve." and el.IDAttribut > 100 and ";
		if(empty($IDQuery)) {
			$requeteRem .= "(Date between '".date('Y-m-d', $lundi)."' and '".date('Y-m-d', $vendredi)."'))";
		} else {
			if(!empty($lundiMin)) {
				$requeteRem .= "(Date between '".$lundiMin."' and '".$vendrediMax."'))";
			} else {
				// force aucun résultat -> todo
				$requeteRem .= "Date is null)";
			}
		}
	}
	...
	*/
	...
	if(hasAdminRigth()) {
		...
		echo "<tr newremarque$last='1' style='display:none' onclick='toggle(\"newremarque$last\")'><td valign='top'><select name='TypeRemarque$last' onclick='limitEvent(event)'><option value='1'>CPG - Remarque</option><option value='2'>JRN - A corriger</option>";
		echo $option;
		$ajd = time();
		if($ajd > $vendredi) {
			$dateajout = $vendredi;
		...
		echo "</select></td><td valign='top'><input name='DateRemarque$last' value='".date('d.m.Y',$dateajout)."' size='8' maxlength='10' onclick='limitEvent(event)'></input></td><td></td><td><textarea name='Remarque$last' COLS=60 ROWS=20 onclick='limitEvent(event)'></textarea></td><td valign='top'><input type='button' name='AjoutRem' value='Ajouter' onclick='submitRemarque(\"$last\",\"ajout\")'></input></td></tr>";
		echo "<tr><td colspan='5' valign='bottom' valign='bottom' bgColor='#5C5C5C'></td></tr>";
		echo "<tr newremarque$last='1'><td colspan='4'></td><td align='center'><img src='/iconsFam/comment_add.png'  onclick='toggle(\"newremarque$last\")' onmouseover=\"Tip('Ajouter une remarque dans le suivi')\" onmouseout='UnTip()'></td></tr>";
	}
	if($showtableSuivi) {
		echo "</table>";
		echo "</div><br><br>";
...
if(!empty($typeEvaluation)) {
	if(($vue==1 && $modeEvaluation=="hebdo") || ($vue==2 && $modeEvaluation=="theme" && $IDQuery!=1)) {
		...
		if($semestreAct==1) {
			if($noSemaine>30) {
				$requeteMoy .= " and (NoSemaine between 30 and ".$noSemaine.")";
			} else {
				$requeteMoy .= " and (NoSemaine > 30 or NoSemaine <= ".$noSemaine.")";
			}
		} else {
			$requeteMoy .= " and (NoSemaine between ".$noSemaineSem2." and ".$noSemaine.")";
		}
		...
		foreach ($compEval as $key => $value) {
			...
			// on part sur le principe que les moyennes existent pour toutes les compétences
		...
		if(!empty($validation)) {
			if($typeEvaluation=='abcd') {
				echo niveauMoy($moyAPP / $pondTot);
			} else {
				echo noteMoy($moyAPP / $pondTot);
			}
		}
		...
		if(hasAdminRigth() || !empty($validation)) {
			...
			if(!empty($validation)) {
				if($typeEvaluation=='abcd') {
					echo niveauMoy($moyMAI / $pondTot);
				} else {
					echo noteMoy($moyMAI / $pondTot);
				}
			}
			...
			if(!empty($validation)) {
				...
				if(hasAdminRigth() && $nbrNotes==count($compEval)*2 && $resp==$_SESSION['user_login']) {
