...
if(!empty($IDTheme)) {
	...
	// recherche des heures / jour travaill�s sur le th�me
...
function Header()
{
	...
	if(empty($IDTheme) || $tri!=0) {
		...
		if($noSemaine>30) {
			$this->Write(0,"Ann�e: ".$annee."/".($annee+1).$txtSem);
		} else {
			$this->Write(0,"Ann�e: ".($annee-1)."/".$annee.$txtSem);
		}
	}
    $this->SetXY(130,23.5);
	if($only!="eval") {
		$this->Write(0,"Heures consacr�es: ".$strHeures);
	} else {
...
if($only!="eval") {
	...
	while ($ligne = mysqli_fetch_assoc($resultat)) {
		...
		// remplacer les caract�re wiki
		$tok = $ligne['Commentaires'];
		$tok = preg_replace("/'''(.*?)'''/", '$1', $tok);
		$tok = preg_replace('/\* (.*?)\n/', '- $1', $tok);
		$tok = preg_replace('/# (.*?)\n/', '- $1', $tok);
		// isoler les diff�rentes lignes du commentaire
		$tok = strtok($tok, "\r\n");
		$found = 0;
		while ($tok !== false) {
			$debut=0;
...
// �valuation: si vue semaine et config eval hebdo ou si vue theme et impression eval activ�e
if((empty($IDTheme)&&$modeEvaluation=="hebdo")||$only=="eval") {
	...
	if(!empty($resultat)&&mysqli_num_rows($resultat)>0) {
		while ($ligne = mysqli_fetch_assoc($resultat)) {
			// ne pas afficher les entr�es non valid�es pour l'apprenti
	...
	if(!empty($dateValidation)) {
		...
		$PDF->Write(0,"Evaluation valid�e le ".date('d.m.Y', strtotime($dateValidation))." par ".$respValidation);
	}
}
