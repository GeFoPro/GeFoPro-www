...
echo ">Termin�s</option></select></td></tr></table><br>\n";
echo "<div id='corners'>";
echo "<div id='legend'>ToDo</div>";
echo "<table id='hor-minimalist-b' width='100%'>\n";
echo "<tr><th width='100'>Date</th><th width='470'>Texte</th><th width='100' align='center'>D�lai</th><th width='100' align='center'>Responsable</th><th width='100' align='center'>Status</th><th></th></tr>";
// recherche des todos
...
while ($ligne = mysqli_fetch_assoc($resultat)) {
	...
	if($ligne['IDStatus']==1 || ($ligne['IDStatus']==2 && strtolower($_SESSION['user_id']) != $ligne['userid'])) {
		echo "<a href='todo.php?IDTodo=$ligne[IDTodo]&action=charge'><img src='/iconsFam/user_add.png' align='absmiddle' onmouseover=\"Tip('Prendre en charge la t�che')\" onmouseout='UnTip()'></a> ";
	} else {
	...
	if($ligne['IDStatus']==2 && strtolower($_SESSION['user_id']) == $ligne['userid']) {
		echo "<a href='todo.php?IDTodo=$ligne[IDTodo]&action=accept'><img src='/iconsFam/accept.png' align='absmiddle' onmouseover=\"Tip('T�che termin�e')\" onmouseout='UnTip()'></a> ";
		echo "<a href='todo.php?IDTodo=$ligne[IDTodo]&action=annule'><img src='/iconsFam/delete.png' align='absmiddle' onmouseover=\"Tip('Annuler la t�che')\" onmouseout='UnTip()'></a> ";
	} else {
		echo "<img src='/iconsFam/empty.png' align='absmiddle'> ";
		echo "<img src='/iconsFam/empty.png' align='absmiddle'> ";
	}
...
echo "<tr newTodo='1'><td colspan='5'></td><td align='right'><img src='/iconsFam/add.png' onmouseover=\"Tip('Ajouter une t�che')\" onmouseout='UnTip()' onclick='toggle(\"newTodo\");' align='absmiddle'></td></tr>";
echo "<tr newTodo='1' style='display:none'><td colspan='6' valign='bottom' height='30'><b>Nouvelle entr�e:<b></td></tr>";
