...
// liste des ids des apprentis, tri�es par classe et par nom
$listeIds = array();
...
// si APP pas charg� l'effectuer maintenant
...
//	echo "<br><center><b>Contenu non autoris�.</b></center>";
//} else {
/* ann�e en cours */
$annee = date('Y');
$mois = date('m');
...
if(!$modeHTML) {
	...
	echo "<br><table width='100%'><tr><td><!-- h2>&nbsp;Liste �l�ves $app_section $annee - $anneePlus</h2 --></td><td align='right'>Mode d'affichage <select id='modeAff' name='modeAff' onChange='submit();'>";
	if($modeAff==0) {
	...
	// ajout d'une ligne pour interrogation des anciennes ann�es (id=100)
	...
	$linkDB = "";
	if($connexion!=null) {
		$linkDB = "<a href='atelier.php?modeHTML&modeLocal=1'><img src='/iconsFam/database_refresh.png' onmouseover=\"Tip('D�sactiver GDN')\" onmouseout='UnTip()'></a>";
	} else {
	...
	echo "<div class='post'><div id='corners'><div id='legend'>Liste �l�ves $app_section $annee - $anneePlus</div><table id='hor-minimalist-b' width='100%'><tr><th colspan='2'><a href='atelier.php'><img src='/iconsFam/page_excel.png' onmouseover=\"Tip('Liste Excel')\" onmouseout='UnTip()'></a>";
	//echo "<a href='syncGDN.php'><img src='/iconsFam/arrow_refresh.png' onmouseover=\"Tip('Synchro local-GDN')\" onmouseout='UnTip()'></a> $linkDB ";
	echo "<a href='../detail/detailEleve.php'><img src='/iconsFam/add.png' onmouseover=\"Tip('Ajouter �l�ve')\" onmouseout='UnTip()'></a></th><th></th><th align='center'>Vestiaire</th><th align='center'>Jetons</th><th align='center' width='100'>Cl�</th><th width='100'>Utilisateur/<br>Nom PC</th><!-- th align='center' width='100'>Chaise/<br>Banc</th --><th>Adresse</th><th align='center' width='120'>Tel. Parents/<br>Personnel</th><th></th><th width='100' align='center'>Date de naissance</th><!-- <th>ID GDN</th> --></tr>\n";
}

// gestion des modification sur �l�ve
...
// construction de la liste des classe si 'ancien' � �t� choisi
if(isset($classe)&&$classe==100) {
...
foreach ($configurationATE as $pos => $value) {
	if(empty($classe) || $classe==$pos || $classe==100 || ($classe==101 && strpos($value, $app_section)===0)) {
		// R�cup�rer la liste des adresses e-mail des profs (seulement si la DB de la GDN est accessible)
		$liste_emails_profs = "" ;
		/*
		if($connexion!=null)
		{
		...
		if(!$modeHTML) {
			...
			//echo "<a href='' id='email_eleve_$value'>  <img src='/iconsFam/iconStudent.gif' height='25' align='absmiddle' onmouseover=\"Tip('Email � la classe')\" onmouseout='UnTip()'></a>";
		...
		//while (($data = nextEntry($connexion,$stmt))!=null){
			...
			//	$troisplusun = true;
			//}
			// recherche des donn�es compl�mentaires
	...
	//echo $requete."<br>";
			...
			// ajout dans liste �leves si internes
			...
			if((!empty($classe)&&$classe!=0)||$ligne['IDEntreprise']==1) {
				if(!$modeHTML) {
					...
					// donn�es uniquement de srv-electro
		...
		//echo $requete."<br>";
					...
					$ico = mysqli_fetch_assoc($resultat);
					if($ico!=null) {
						echo " bgcolor='#F8ECE0' onmouseover=\"Tip('Cong� le ".date('d.m.Y', strtotime($ico['Date'])).": ".addslashes($ico['Remarque'])."')\" onmouseout='UnTip()'";
					}
					if($ligne['IDEntreprise']!=1) {
		...
		//echo $requete."<br>";
						...
						$ico = mysqli_fetch_assoc($resultat);
						if($ico!=null) {
							echo "<img src='/iconsFam/rosette.png' onmouseover=\"Tip('D�l�gu� de classe')\" onmouseout='UnTip()'>";
							$cntIcon++;
						}
						if($cntIcon==3) {
							echo "<br>";
						...
						$ico = mysqli_fetch_assoc($resultat);
						if($ico!=null) {
							echo "<img src='/iconsFam/clock_error.png' onmouseover=\"Tip('Horaire bloqu�')\" onmouseout='UnTip()'>";
							$cntIcon++;
						}
						if($cntIcon==3) {
							echo "<br>";
						...
						$ico = mysqli_fetch_assoc($resultat);
						if($ico!=null) {
							echo "<img src='/iconsFam/cog_delete.png' onmouseover=\"Tip('Hors liste t�ches')\" onmouseout='UnTip()'>";
						}
...
//m�morisation de la liste d'�leves en session
...
<div id='corners'><div id='legend'>L�gende</div>
<br><table border='0'>
<!-- tr><td colspan><b>L�gendes:</b></td></tr -->
<tr><td width='5'></td><td>Formation 3 + 1</td><td><img src='/iconsFam/award_star_gold_1.png'></td><td width='100'></td><td>D�l�gu� de classe</td><td><img src='/iconsFam/rosette.png'></td><td width='100'></td><td>Horaire bloqu�</td><td><img src='/iconsFam/clock_error.png'></td><td width='100'></td><td>Stage</td><td><img src='/iconsFam/building_go.png'></td></tr>
<tr><td width='5'></td><td>Inscrit(e) 3 + 1</td><td><img src='/iconsFam/award_star_bronze_2.png'></td><td width='100'></td><td>Cours d'appui</td><td><img src='/iconsFam/flag-english.png' onmouseover="Tip('Anglais')" onmouseout='UnTip()'> <img src='/iconsFam/flag-german.png' onmouseover="Tip('Allemand')" onmouseout='UnTip()'> <img src='/iconsFam/calculator.png' onmouseover="Tip('Maths')" onmouseout='UnTip()'></td><td width='100'></td><td>Hors liste t�ches</td><td><img src='/iconsFam/cog_delete.png'></td></tr>
<tr><td width='5'></td><td>Inscrit(e) 3 + 1, pr�p. MPT</td><td><img src='/iconsFam/award_star_silver_3.png'></td><td width='100'></td><td>Cours pr�p. MPT</td><td><img src='/iconsFam/medal_silver_3.png'></td><td width='100'></td><td>Formation dual</td><td><img src='/iconsFam/building.png'></td></tr>
</table></div>
</div> <!-- post -->
</form>
<?php } // has admin right?>
