...
if(!empty($actionModifier)) {
	if(isset($_SESSION['APP']) && $_SESSION['APP']['LockHoraire']) {
		//$actionModifier = "";
		//$msgLock = "L'horaire est actullement modifi� par un autre utilisateur";
	} else {
...
if(!isset($_POST['noSemaine']) || "" == $_POST['noSemaine']) {
	...
	if(!isset($_SESSION['noSemaine']) || "" == $_SESSION['noSemaine']) {
		...
		//echo "- trouv� en session: ".$_SESSION['noSemaine']."/".$_SESSION['anneeCalc'];
		$noSemaine = $_SESSION['noSemaine'];
		$anneeCalc = $_SESSION['anneeCalc'];
	}
} else {
	//echo "trouv� en POST: ".$_POST['noSemaine']."/".$_POST['anneeCalc'];
	$noSemaine = $_POST['noSemaine'];
...
// cr�ation des �l�ments s�lectionn�s
$checked = array();
...
$errors = array();
// cr�ation des p�riode en erreur
for($jour=1;$jour<=5;$jour++) {
	foreach ($periods as $cell => $value) {
		$periodeOK = false;
		$periodProf = array();
		foreach ($profs as $pos => $value) {
			...
			if(in_array($idCell,$checked)) {
				// au moins un enseignant dans la p�riode -> OK
				$periodeOK = true;
			}
		}
		// si periodOK false -> enregister les cellules en erreur
		if(!$periodeOK) {
			//echo "P�riode $cell erreur<br>";
...
if(empty($actionModifier)) {
	...
	if($cntBoucle>1) {
		echo "<td align='right' onClick='submitSemaineAnnee(0,".$anneeCalc.")'><i><font color='green'>Horaire semaine $noSemaine par d�faut</font></i></td>";
	} else {
		echo "<td align='right'><i><font color='red'>Horaire semaine $noSemaine personnalis�</font></i></td>";
	}
} else {
...
foreach ($periods as $per => $value) {
	if($precedent!=0) {
		echo "<th align='center'>$precedent<br>�<br>$value</th>";
	}
	$precedent = $value;
}
	...
	foreach ($profs as $pos => $value) {
		...
		if(end($periods)!=$value) {
			...
			if(in_array($idCell,$theories)) {
				...
				// incr�menter compteur prof si th�orie
				$time = $time + $dureePeriode[$cell];
...
<?php
// construction du r�capitulatif par prof
$span = $cntProfs+1;
...
if(isMAIAEL()) {
	if(empty($actionModifier)) {
		...
		echo "<b>Astuce: </b>S�lectionner/d�s�lectionner toute la ligne restante en cliquant sur la case � cocher en maintenant la touche Ctrl.<br><br>";
