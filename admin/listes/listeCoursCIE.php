...
if(isset($_GET['effacerCours'])) {
	...
	$resultat =  mysqli_query($connexionDB,$requete);
	// rechercher les �vlauationa et observation pour chaque inscriptions � effacer
...
if(isset($_GET['effacerInscription'])) {
	...
	// rechercher les �vlauations et observation pour chaque inscription � effacer
	$requete = "SELECT IDDocEleve from docelevecie where IDCours=$IDCours";
	$resultat =  mysqli_query($connexionDB,$requete);
	while($ligne = mysqli_fetch_assoc($resultat)) {
		// �valuations
...
if(isset($_GET['effacerEvaluation'])) {
	...
	// rechercher les �vlauations et observation pour chaque inscription concern�e
	$requete = "SELECT IDDocEleve from docelevecie where IDCours=$IDCours";
	$resultat =  mysqli_query($connexionDB,$requete);
	while($ligne = mysqli_fetch_assoc($resultat)) {
		// �valuations
...
echo "<br><table border=0 width='100%'><tr><td><!-- h2>Liste des cours CIE</h2 --></td><td align='right'>Ann�e civile: <select name='annee' onchange='submit();'>".$optionAnnee."</select></td></tr></table>";
echo "<br><div id='corners'>";
echo "<div id='legend'>Liste des cours CIE</div>";
echo "<table id='hor-minimalist-b' width='100%'>\n";
echo "<tr><th width='250'>Cours</th><th width='80'>Version CC</th><th width='150'>Dates du cours</th><th width='50'>Jours</th><th width='100'>Responsable</th><th>Inscrits</th><th>Scann�s</th><th>Eval APP</th><th>Eval MAI</th><th></th></tr>";
...
while($ligne = mysqli_fetch_assoc($resultat)) {
	...
	echo ">".$ligne['pdf']."</td>";
	// recherche des �ventuelles �valuations pr�sentes
	...
	echo "<td align='center'>";
	if($ligne['pdf']==0) {
		echo "<a href='listeCoursCIE.php?effacerCours=".$ligne['IDCours']."' onclick='return confirm(\"Supprimer ce cours ainsi que toutes les �valuations d�j� saisies?\")'><img src='/iconsFam/book_delete.png' align='absmiddle' onmouseover=\"Tip('Supprimer ce cours')\" onmouseout='UnTip()'></a>";
		if($ligne['Inscrits']!=0) {
			echo "<a href='listeCoursCIE.php?effacerInscription=".$ligne['IDCours']."' onclick='return confirm(\"D�sinscrire la classe et supprimer toutes les �valuations d�j� saisies?\")'><img src='/iconsFam/user_delete.png' align='absmiddle' onmouseover=\"Tip('Supprimer les inscriptions')\" onmouseout='UnTip()'></a>";
		}
	}
	else if($ligne['pdf']==$ligne['Inscrits']) {
		// possibilit� de purger les �valuations et observations
		echo "<a href='listeCoursCIE.php?effacerEvaluation=".$ligne['IDCours']."' onclick='return confirm(\"Supprimer les �valuations saisies et ne conserver que les documents scann�s?\")'><img src='/iconsFam/folder_delete.png' align='absmiddle' onmouseover=\"Tip('Supprimer les �valuations')\" onmouseout='UnTip()'></a>";
	}
	echo "</td>";
	echo "</tr>";
}
echo "<tr><td colspan='10' valign='bottom' valign='bottom' bgColor='#5C5C5C'></td></tr>";
echo "<tr><td colspan='3'><b>Total</b></td><td align='center'>".sprintf("%01.1f",$totalJour)."</td><td colspan='5'></td><td align='center'><img src='/iconsFam/add.png' onmouseover=\"Tip('Ajouter un cours')\" onmouseout='UnTip()' onclick='toggle(\"newCours\");' align='absmiddle'><img src='/iconsFam/user_add.png' onmouseover=\"Tip('Inscrire � un cours')\" onmouseout='UnTip()' onclick='toggle(\"insCours\");' align='absmiddle'></td></tr>";
...
echo "<tr insCours='1' style='display:none'><td colspan='8'>Inscrire la classe <select name='classe'><option value=''></option><option value='".$app_section." 1'>".$app_section." 1</option><option value='".$app_section." 2'>".$app_section." 2</option><!-- option value='".$app_section." 3'>".$app_section." 3</option --></select> pour le cours s�lectionn� ci-dessus</td><td colspan='2' align='right'><input type='submit' name='inscrire' value='Inscrire'></td></tr>";
echo "</table></div><br>";

// construction de la liste des documents
echo "<br><table border=0 width='100%'><tr><td><!-- h2>Liste des documents \"Contr�le de comp�tences\" et contenu</h2 --></td><td align='right'></td></tr></table>";
echo "<br><div id='corners'>";
echo "<div id='legend'>Liste des documents \"Contr�le de comp�tences\" et contenu</div>";
echo "<table id='hor-minimalist-b' width='100%'>\n";
...
while($ligne = mysqli_fetch_assoc($resultatH)) {
	...
	if(!empty($ligne['Numero'])) {
		...
		echo "<td align='right'>";
		//if($ligne['eval']==0) {
			echo "<a href='listeCoursCIE.php?effacerCompetence=".$ligne['IDCompetence']."&IDDoc=".$doc."'><img src='/iconsFam/table_row_delete.png' align='absmiddle' onmouseover=\"Tip('Supprimer cette comp�tence')\" onmouseout='UnTip()'></a>";
		//}
		echo "</td></tr>";
	}
}
...
echo "<tr newDoc='1' style='display:none' ><td colspan='5' valign='bottom' height='30'><b>Ajouter un document de contr�le de comp�tences:</b></td></tr>";
