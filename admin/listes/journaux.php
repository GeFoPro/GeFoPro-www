...
if($modeEvaluation=="hebdo") {
	// si mode hebdo on affiche les 4 derni�res ann�e (bidouille en attendant)
	$annee = $annee-5;
}
...
if($vue==2) {
	...
	//echo "<td align='right'>Vue: <select name='vue' onChange='toggle(\"vueTable\")'><option value='1'>Par th�me</option><option value='2' ";
	echo "<td align='right'>Vue: <select name='vue' onChange='document.getElementById(\"myForm\").submit();'><option value='2'>Par th�me</option><option value='1' ";
	...
	echo "</h2></td>";
	echo "<td align='right'>Vue: <select name='vue' onChange='document.getElementById(\"myForm\").submit();'><option value='2'>Par th�me</option><option value='1' selected>Par semaine</option></select></td></tr>";
	//echo "<td align='right'>Vue: <select name='vue' onChange='toggle(\"vueTable\")'><option value='1'>Par th�me</option><option value='2' selected>Par semaine</option></select></td></tr>";
...
if($vue==2) {
	...
	echo "<tr><th width='50'>Th�me</th><th width='150'>Nom</th><th width='50' align='center'>Nbr. jour</th><th width='80' align='center'>Nbr. heures</th>";
	if($modeEvaluation=="hebdo") {
		echo "<th></th>";
	} else {
		echo "<th>Eval. semestre ".$semestreAct."</th>";
	}
	//echo "<th width='100'>Motivation</th><th width='100'>Progrets</th><th width='100'>Difficult�s</th><th width='100'>Notes</th>";
...
//LEFT JOIN evaluation ev on jo.IDTheme=ev.IDTheme and jo.IDEleve=ev.IDEleve and (ev.Annee is null OR ev.Annee = ".$annee.")
	...
	while ($ligne = mysqli_fetch_assoc($resultat)) {
		...
		//echo "<td>";
		//echo <img src='/iconsFam/exclamation.png' align='absmiddle' onmouseover=\"Tip('Avertir oubli �valuation')\" onmouseout='UnTip()'>";
		//if(!empty($ligne['NoteTechnique'])) {
		//	echo $ligne['NoteTechnique']." | ".$ligne['NoteApplication']." | ".$ligne['NoteRendement']." | ".$ligne['NoteSavoirEtre'];
		//}
		//echo "</td>";
		echo "<td align='center'>";
		// recherche journaux non valid�s
		...
		if($ligneEntrees[0]!=0) {
			echo "<img src='/iconsFam/error.png' align='absmiddle' onmouseover=\"Tip('".$ligneEntrees[0]." entr�e(s) dans les journaux non valid�e(s) avant le ".date("d.m.Y",$lundiSemEncours)."')\" onmouseout='UnTip()'>";
		} else {
		...
		//	echo " <img src='/iconsFam/bullet_red.png' align='absmiddle' onmouseover=\"Tip('Evaluation non valid�e')\" onmouseout='UnTip()'>";
		//} else {
		//	if(empty($ligne['Annee'])) {
		//		echo " <img src='/iconsFam/bullet_orange.png' align='absmiddle' onmouseover=\"Tip('Evaluation valid�e, pas assign�e � une p�riode')\" onmouseout='UnTip()'>";
		//	} else {
		//		echo " <img src='/iconsFam/bullet_green.png' align='absmiddle' onmouseover=\"Tip('Evaluation valid�e, ann�e ".$ligne['Annee']."/".($ligne['Annee']+1)."')\" onmouseout='UnTip()'>";
		//	}
		//}
		//echo "</a>";
		echo "</td></tr>";
	...
	while ($ligne = mysqli_fetch_assoc($resultat)) {
		if($lastClasse!=$ligne['Classe'] || $lastEleve!=$ligne['IDGDN']) {
			// nouvelle classe ou nouvel �l�ve -> terminer la ligne pr�c�dente
			if($lastEleve!=0) {
				if($cumulJour!=0) {
				...
				// terminer les jours non renseign�s
				while($jour<=$vendredi) {
					echo "<td valign='top' align='center'>-</td>";
					$jour += 86400;
				}
				// terminer la ligne pr�c�dante
				...
				if($cntlock!=0 && $cntunlock==0) {
					echo " <img src='/iconsFam/bullet_green.png' align='absmiddle' onmouseover=\"Tip('Journal valid�')\" onmouseout='UnTip()'>";
				} else if($cntlock!=0 && $cntunlock==0) {
					echo " <img src='/iconsFam/bullet_orange.png' align='absmiddle' onmouseover=\"Tip('Journal partiellement valid�')\" onmouseout='UnTip()'>";
				} else {
					echo " <img src='/iconsFam/bullet_red.png' align='absmiddle' onmouseover=\"Tip('Journal non valid�')\" onmouseout='UnTip()'>";
				}
				echo "</td></tr>";
			}
		}
		if($lastClasse!=$ligne['Classe']) {
			// nouvelle classe � afficher
			echo "<tr><td colspan='10' valign='bottom' valign='bottom' bgColor='#5C5C5C'></td></tr>";
	...
	if($lastEleve!=0) {
		...
		if(substr($lastClasse, -1)==4 && ($nbrjour<4 || $nbrjour>4)) {
			//echo "<td align='right'><img src='/iconsFam/error.png' align='absmiddle' onmouseover=\"Tip('Semaine incompl�te')\" onmouseout='UnTip()'>";
			$msgErreur .= "Semaine incompl�te<br>";

		} else if($nbrjour<3 || $nbrjour>3) {
			//echo "<td align='right'><img src='/iconsFam/error.png' align='absmiddle' onmouseover=\"Tip('Semaine incompl�te".$msgErreur."')\" onmouseout='UnTip()'>";
			$msgErreur .= "Semaine incompl�te<br>";
		} else {
			//echo "<td align='right'>";
		}
