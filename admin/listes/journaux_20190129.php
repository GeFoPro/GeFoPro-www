...
if($vue==1) {
	...
	echo "<td align='right'>Vue: <select name='vue' onChange='document.getElementById(\"myForm\").submit();'><option value='1'>Par thème</option><option value='2' ";
	...
	echo "<td width='33%'></td><td align='center' width='33%'><h2><img src='/iconsFam/resultset_previous.png' onClick='submitSemaineAnnee(".$noSemMoinsUn.",".$anneeMoinsUn.")'>Semaine ".$noSemaine;
	...
	echo "</h2></td>";
	echo "<td align='right'>Vue: <select name='vue' onChange='document.getElementById(\"myForm\").submit();'><option value='1'>Par thème</option><option value='2' selected>Par semaine</option></select></td></tr>";
...
$requete = "SELECT IDTheme, TypeTheme, NomTheme, Objectif, IDGDN, Nom, Prenom, sum( heures ) AS heures, count( heures ) AS jours
FROM (
SELECT  NomTheme, jo.IDTheme as IDTheme,TypeTheme,Objectif,jo.IDEleve as IDGDN, Nom, Prenom, DateJournal, sum( Heures ) AS heures
FROM elevesbk
...
//LEFT JOIN evaluation ev on jo.IDTheme=ev.IDTheme and jo.IDEleve=ev.IDEleve and (ev.Annee is null OR ev.Annee = ".$annee.") 
	...
	while ($ligne = mysql_fetch_assoc($resultat)) {
		...
		echo "<tr onclick='document.location.href=\"../detail/activites.php?from=journaux&triPeriode=1&idEleve=".$ligne['IDGDN']."&nom=".$ligne['Nom']."&prenom=".$ligne['Prenom']."&IDTheme=".$ligne['IDTheme']."\"'><td></td><td valign='top'>".$ligne['Nom']." ".$ligne['Prenom']."</td><td valign='top' align='center'>".$ligne['jours']."</td><td align='center' valign='top'>".sprintf("%5.1f",$ligne['heures'])."h</td>";
	...
	$requete = "SELECT Classe, elbk.IDGDN, Nom, Prenom, DateJournal, DateValidation, Heures FROM elevesbk elbk join eleves el on elbk.IDGDN=el.IDGDN and el.IDEntreprise=1 left join journal jo on elbk.IDGDN=jo.IDEleve and (jo.DateJournal between '".date('Y-m-d', $lundi)."' and '".date('Y-m-d', $vendredi)."') where (".$whereclasse.") order by Classe desc ,Nom,Prenom,DateJournal";
	$resultat =  mysql_query($requete);
	//echo $requete;
	echo "<br><div id='corners'>";
