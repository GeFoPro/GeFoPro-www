...
if(!empty($themeTri)) {
	// tri sur theme, toutes ann�es et etat confindus
	$filtre = 10;
	$anneeTri = '%';
...
if(isset($_POST['actionProjet'])) {
	if($_POST['actionProjet']=='newAttrib') {
		...
		//echo $requete;
    		$resultat =  mysqli_query($connexionDB,$requete);
		// ajout d'une ligne dans le suivi pour d�but de projet
		//$requete = "insert into suiviprojet (IDProjet, DateSaisie, RemarqueSuivi) values (".$projet.", \"".date('Y-m-d')."\", \"D�but de projet\")";
		$requete = "insert into remarquesuivi (IDEleve, IDTheme, DateSaisie, Remarque, TypeRemarque) values (".$eleve.", ".$theme.",\"".date('Y-m-d')."\", \"D�but de projet\",1)";
		//echo $requete;
    		$resultat =  mysqli_query($connexionDB,$requete);
...
echo "<br><font color='#FF0000'><b>Attention: v�rifier avec l'apprenti qu'il ait bien fini de saisir ses journaux ainsi que son auto-�valuation avant de cl�turer un projet!</b></font>";
echo "</center>";


// construction de la liste des �l�ves
...
//if($filtre==10) {
	...
	if(empty($themeTri)) {
		echo "Ann�e: <select name='annee' onchange='submit()'><option value='%'>Tous</option>".$optionAnnee."</select>";
	}
	if($anneeTri!='%') {
...
if(!empty($themeTri)) {
	echo "<option value='12' selected>S�lection</option>";
}
...
echo "<tr><th width='300'>Nom projet</th><th width='10'></th><th>Attribu� �</th><th width='80'>D�but</th><th width='80'>Fin</th><th width='80'>Type</th><th width='80'>Etat</th><th width='10'>Suivi</th><th width='10'>Not�</th><th width='10'></th></tr>";
...
while ($ligne = mysqli_fetch_assoc($resultat)) {
	...
	if($lastID!=$ligne['IDTheme']) {
		...
		if($ligne['IDEtatProjet']==0||$ligne['IDEtatProjet']==1||$ligne['IDEtatProjet']==4) {
			// ajout apprenti si attente, en cours ou � reprendre
			echo "<img src='/iconsFam/user_add.png' align='absmiddle' onmouseover=\"Tip('Attribuer � un �l�ve')\" onmouseout='UnTip()' onclick='toggle(\"newattrib$idTheme\");'>";
		}
		echo "</td>";
		if(empty($ligne['Nom'])) {
		...
		if(!isset($ligne['IDEtatProjet'])) {
			...
			echo "<img src='/iconsFam/control_end.png' align='absmiddle' onmouseover=\"Tip('Marquer comme termin�')\" onmouseout='UnTip()' onclick='submitUpdateProj(\"$idTheme\",\"end\")'>";
		}
		echo "</td></tr>";
	...
	if(!empty($ligne['Nom'])) {
		...
		if($ligne['IDEtatProjet']==0) {
			echo "<img src='/iconsFam/control_play.png' align='absmiddle' onmouseover=\"Tip('D�marrer le projet')\" onmouseout='UnTip()' onclick='submitUpdateProjID(\"$idProjet\",\"$idTheme\",\"$idEleve\",\"start\")'>";
		...
		//if($lastID==$ligne['IDTheme']&&$lastEtat!=$ligne['IDEtatProjet']) {
			// �tat diff�rents sur m�me th�me -> on afficher les options pour terminer
	...
	if($lastID==$ligne['IDTheme']&&$lastEtat!=$ligne['IDEtatProjet']) {
		// si �tat diff�rent dans le m�me th�me, on r�initialise le dernier �tat afin que le prochain soit affich�
		$lastEtat = 0;
	} else {
